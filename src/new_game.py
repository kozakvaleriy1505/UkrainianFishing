    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                x, y = event.pos
                if 300 <= x <= 600 and 400 <= y <= 460:
                    nickname = input_text.strip()
                    if nickname == "":
                        print("[!] Ім’я профілю не може бути порожнім!")
                        continue

                    print(f"▶ Створення профілю: {nickname}, складність: {difficulties[selected]}")
